...
const log = createChildLogger("gemini-cli");
...
export class GeminiCliRunner implements AgentRunner {
  async run(config: AgentRunnerConfig): Promise<AgentResult> {
    ...
    log.info({ agent: config.agentSlug, cwd: config.projectPath }, "Spawning Gemini CLI");

    return new Promise((resolve) => {
      try {
        const args = ["-p", config.input, "--output-format", "text"];
